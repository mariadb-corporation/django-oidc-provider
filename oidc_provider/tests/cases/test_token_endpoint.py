...
class TokenTestCase(TestCase):
    ...
    def _post_request(self, post_data, extras={}):
        ...
        request = self.factory.post(
            url, data=urlencode(post_data), content_type="application/x-www-form-urlencoded", **extras
        )
    ...
    def test_default_setting_does_not_allow_grant_type_password(self):
        ...
        response = self._post_request(post_data=post_data, extras=self._password_grant_auth_header())
    ...
    def test_password_grant_get_access_token_with_scope(self):
        response = self._post_request(
            post_data=self._password_grant_post_data(), extras=self._password_grant_auth_header()
        )
    ...
    def test_password_grant_get_access_token_invalid_user_credentials(self):
        ...
        response = self._post_request(post_data=invalid_post, extras=self._password_grant_auth_header())
    ...
    def check_password_grant(self, scope):
        response = self._post_request(
            post_data=self._password_grant_post_data(scope), extras=self._password_grant_auth_header()
        )
    ...
    @override_settings(
        OIDC_GRANT_TYPE_PASSWORD_ENABLE=True, AUTHENTICATION_BACKENDS=("oidc_provider.tests.app.utils.TestAuthBackend",)
    )
    ...
    def _refresh_request(self, elapsed_time):
        ...
        response_dic1 = json.loads(response.content.decode("utf-8"))
        # Use refresh token to obtain new token
        post_data = self._refresh_token_post_data(response_dic1["refresh_token"], code.scope)
    ...
    def test_client_authentication(self):
        ...
        self.assertNotIn(
            "invalid_client", response.content.decode("utf-8"), msg="Client authentication fails using HTTP Basic Auth."
        )
    ...
    @override_settings(OIDC_IDTOKEN_PROCESSING_HOOK=("oidc_provider.tests.app.utils.fake_idtoken_processing_hook3"))
    def test_additional_idtoken_processing_hook_scope_available(self):
        ...
        self.assertEqual(
            id_token.get("scope_of_token_passed_to_processing_hook"), ["openid", "email", "profile", "dummy"]
        )

    @override_settings(OIDC_IDTOKEN_PROCESSING_HOOK=("oidc_provider.tests.app.utils.fake_idtoken_processing_hook4"))
    ...
    def test_requested_scope(self):
        ...
        response = self._post_request(
            post_data=self._password_grant_post_data(["email"]), extras=self._password_grant_auth_header()
        )
        ...
        response = self._post_request(
            post_data=self._password_grant_post_data(["email", "openid"]), extras=self._password_grant_auth_header()
        )
        ...
        response = self._post_request(post_data=self._client_credentials_post_data(["openid", "invalid_scope"]))
        ...
        # happy path: multiple scopes
        response = self._post_request(post_data=self._client_credentials_post_data(["email", "openid"]))
